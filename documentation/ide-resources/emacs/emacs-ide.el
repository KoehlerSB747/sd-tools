...
(global-set-key "\C-c\C-b" 'add-braces)
...
(defun build-execute-java-command (&optional arg)
  (let* ((command "java -Xmx640m ")
         (cp1 "-classpath `cpgen-uni ")
         (cp1t "-classpath `cpgen-uni -b ")
         ...
         (istest (or (string-match "\\.Test" javaclass) (string-match "^.*Test$" javaclass)))
         )
    (concat command
            (if istest cp1t cp1)
            cp2 cp3
            (if istest test-command "")
            javaclass)
    ))
